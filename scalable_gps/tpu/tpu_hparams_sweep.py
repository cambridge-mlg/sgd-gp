    ...
    try:
        ...
        run_experiment_command = [
            ...
            "--command", "git clone https://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@github.com/jandylin/scalable-gaussian-processes.git; "
                        ...
                        f"python scripts/obtain_mean.py 9835d6db89010f73306f92bb9a080c9751b25d28 --config configs/default.py:{dataset_name} --config.model_name sgd --config.wandb.log --config.wandb.name {dataset_name}_hparamsweep_lr={lr}_bs={batch_size} --config.train_config.learning_rate {lr} --config.train_config.batch_size {batch_size} --noconfig.wandb.log_artifact",
            "--", "-t"
        ]
        ...
        print(f"An error occurred while running the experiment and tpu-hparamsweep-{dataset_name}-{lr}-{batch_size}: {e}")
        raise
