...
def regularizer(params, target, K, noise_scale):
    ...
    return .5 * (params - target).T @ K @ (params - target)


# TODO: pmap over M.
def regularizer_grad_sample(params, key, M, x, target, feature_fn, noise_scale, recompute_features=True):
...
def loss_fn(params: Array, target_tuple: TargetTuple, K: Array, noise_scale):
    ...
    return error(params, target_tuple.error_target, K) + regularizer(params, target_tuple.regularizer_target, K, noise_scale)
...
def draw_prior_function_sample(
    feature_key: PRNGKey, prior_function_key: PRNGKey, M: int, x: Array, feature_fn: Callable, K: Optional[Array]=None, 
    use_chol: bool = False, chol_eps: float = 1e-5):
